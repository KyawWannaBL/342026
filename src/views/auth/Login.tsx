...
export default function LoginPage() {
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center overflow-hidden bg-[#05080F]">
      ...
      {/* 1. MP4 BACKGROUND - Pointing to your exact public file */}
          ...
          <CardContent className="p-8">
            ...
            {step === 'login' && (
              <form onSubmit={handleLogin} className="space-y-5">
                <div className="space-y-4">
                  <div className="relative">
                    ...
                    <Input type="email" required value={email} onChange={(e) => setEmail(e.target.value)}
                      className="bg-black/50 border-white/10 text-white h-14 rounded-xl pl-12 focus:border-emerald-500/50"
                ...
                <div className="flex justify-end">
                  <button type="button" onClick={() => { setStep('forgot'); clearMessages(); }} className="text-[10px] text-slate-400 hover:text-emerald-400 font-bold uppercase transition-colors">
                    {t('Forgot Access Code?', 'စကားဝှက် မေ့နေပါသလား?')}
                  </button>
                </div>

                <Button type="submit" disabled={loading} className="w-full h-14 bg-emerald-600 hover:bg-emerald-500 text-white font-black tracking-widest uppercase rounded-xl flex items-center justify-center gap-2 transition-all">
            ...
            {step === 'signup' && (
              <form onSubmit={handleSignUp} className="space-y-5">
                <button type="button" onClick={() => setStep('login')} className="flex items-center gap-2 text-[10px] text-slate-400 hover:text-white font-bold uppercase mb-4">
                ...
                <div className="space-y-4">
                  <div className="relative">
                    ...
                    <Input type="email" required value={email} onChange={(e) => setEmail(e.target.value)}
                      className="bg-black/50 border-white/10 text-white h-14 rounded-xl pl-12" placeholder={t("New Email", "အီးမေးလ်အသစ်")} />
                  </div>
                  <div className="relative">
                    <Lock className="absolute left-4 top-4 h-5 w-5 text-slate-400" />
                    <Input type="password" required value={password} onChange={(e) => setPassword(e.target.value)}
                      className="bg-black/50 border-white/10 text-white h-14 rounded-xl pl-12" placeholder={t("New Password", "စကားဝှက်အသစ်")} />
                  </div>
                </div>
            ...
            {step === 'forgot' && (
              <form onSubmit={handleForgot} className="space-y-5">
                ...
                <div className="relative">
                  <Mail className="absolute left-4 top-4 h-5 w-5 text-slate-400" />
                  <Input type="email" required value={email} onChange={(e) => setEmail(e.target.value)}
                    className="bg-black/50 border-white/10 text-white h-14 rounded-xl pl-12" placeholder={t("Corporate Email", "အီးမေးလ်")} />
                </div>
              ...
              <form onSubmit={handleForceChangePassword} className="space-y-5">
                <div className="p-4 bg-amber-500/10 border border-amber-500/30 rounded-xl mb-6">
                  <p className="text-xs text-amber-400 font-bold leading-relaxed">
                    {t('System requires a password update before granting access.', 'ဝင်ရောက်ခွင့်မပြုမီ စကားဝှက်အသစ်ပြောင်းရန် လိုအပ်ပါသည်။')}
                  </p>
                </div>
                <div className="space-y-4">
                  <div className="relative">
                    ...
                    <Input type="password" required value={newPassword} onChange={(e) => setNewPassword(e.target.value)}
                      className="bg-black/50 border-amber-500/30 text-white h-14 rounded-xl pl-12 focus:border-amber-500" placeholder={t("New Password", "စကားဝှက်အသစ်")} />
                  </div>
                  <div className="relative">
                    <CheckCircle2 className="absolute left-4 top-4 h-5 w-5 text-slate-400" />
                    <Input type="password" required value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)}
                      className="bg-black/50 border-amber-500/30 text-white h-14 rounded-xl pl-12 focus:border-amber-500" placeholder={t("Confirm Password", "စကားဝှက် အတည်ပြုပါ")} />
                  </div>
                </div>
